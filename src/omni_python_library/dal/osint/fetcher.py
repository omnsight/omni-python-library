...
class OsintDataFetcher(ArangoOperator):
    ...
    def get_relation(
        self, id: str, owner: str = None, roles: List[str] = [], in_pending: bool = False
    ) -> Optional[Relation]:
        return self._get(Relation, id, owner=owner, roles=roles, in_pending=in_pending)

    def get_event(self, id: str, owner: str = None, roles: List[str] = [], in_pending: bool = False) -> Optional[Event]:
