import unittest

from omni_python_library import init_omni_library
from omni_python_library.dal.osint_data_access_layer import OsintDataAccessLayer
from omni_python_library.models.osint import WebsiteMainData
from omni_python_library.utils.config import UserRole
from omni_python_library.utils.errors import PermissionDeniedError


class TestWebsiteCRUD(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        init_omni_library()

    def test_website_crud(self):
        # Create
        website_data = WebsiteMainData(
            url="http://example.com",
            read=[UserRole.ADMIN],
            write=[UserRole.ADMIN],
        )

        created = OsintDataAccessLayer().create_website(website_data, owner="test_user", roles=[UserRole.ADMIN])
        self.assertIsNotNone(created)
        self.assertEqual(created.url, "http://example.com")

        # Read
        fetched = OsintDataAccessLayer().get_website(created.id, owner="test_user", roles=[UserRole.ADMIN])
        self.assertIsNotNone(fetched)
        self.assertEqual(fetched.url, "http://example.com")

        # Update
        updated = OsintDataAccessLayer().update_website(
            created.id, WebsiteMainData(url="http://example.org"), owner="test_user", roles=[UserRole.ADMIN]
        )
        self.assertEqual(updated.url, "http://example.org")

        # Read again
        fetched_updated = OsintDataAccessLayer().get_website(created.id, owner="test_user", roles=[UserRole.ADMIN])
        self.assertEqual(fetched_updated.url, "http://example.org")

        # Delete
        deleted = OsintDataAccessLayer().delete_entity(created.id, owner="test_user", roles=[UserRole.ADMIN])
        self.assertTrue(deleted)

        # Verify Delete
        fetched_deleted = OsintDataAccessLayer().get_website(created.id, owner="test_user", roles=[UserRole.ADMIN])
        self.assertIsNone(fetched_deleted)

    def test_website_crud_with_pending(self):
        # Create
        website_data = WebsiteMainData(
            url="http://pending.com",
            read=[UserRole.ADMIN],
            write=[UserRole.ADMIN],
        )
        created = OsintDataAccessLayer().create_website(website_data, owner="test_user_pending", roles=[UserRole.ADMIN])
        self.assertIsNotNone(created)
        self.assertEqual(created.url, "http://pending.com")

        # Update with in_pending=True
        OsintDataAccessLayer().update_website(
            created.id,
            WebsiteMainData(url="http://pending.org"),
            owner="test_user_pending",
            roles=[UserRole.ADMIN],
            in_pending=True,
        )

        # Read with in_pending=False (default) - should get original data
        fetched_not_pending = OsintDataAccessLayer().get_website(
            created.id, owner="test_user_pending", roles=[UserRole.ADMIN]
        )
        self.assertEqual(fetched_not_pending.url, "http://pending.com")

        # Read with in_pending=True - should get merged data
        fetched_pending = OsintDataAccessLayer().get_website(
            created.id, owner="test_user_pending", roles=[UserRole.ADMIN], in_pending=True
        )
        self.assertEqual(fetched_pending.url, "http://pending.org")

        # Perform a final update (in_pending=False)
        final_update = OsintDataAccessLayer().update_website(
            created.id, WebsiteMainData(url="http://final.com"), owner="test_user_pending", roles=[UserRole.ADMIN]
        )
        self.assertEqual(final_update.url, "http://final.com")

        # Read again, should be permanently updated
        fetched_final = OsintDataAccessLayer().get_website(
            created.id, owner="test_user_pending", roles=[UserRole.ADMIN]
        )
        self.assertEqual(fetched_final.url, "http://final.com")

        # Clean up
        OsintDataAccessLayer().delete_entity(created.id, owner="test_user_pending", roles=[UserRole.ADMIN])

    def test_website_crud_permission_denied(self):
        # Create a website with a specific owner and roles
        website_data = WebsiteMainData(
            url="http://secure.com",
            read=[UserRole.ADMIN],
            write=[UserRole.ADMIN],
        )
        created = OsintDataAccessLayer().create_website(website_data, owner="owner_user", roles=[UserRole.ADMIN])
        self.assertIsNotNone(created)

        # Test creating with insufficient permissions
        with self.assertRaises(PermissionDeniedError):
            OsintDataAccessLayer().create_website(website_data, owner="non_admin_user", roles=[UserRole.USER])

        # Test reading with wrong owner and no matching roles
        with self.assertRaises(PermissionDeniedError):
            OsintDataAccessLayer().get_website(created.id, owner="another_user", roles=[UserRole.USER])

        # Test updating with wrong owner and no matching roles
        with self.assertRaises(PermissionDeniedError):
            OsintDataAccessLayer().update_website(
                created.id,
                WebsiteMainData(url="http://secure-updated.com"),
                owner="another_user",
                roles=[UserRole.USER],
            )

        # Test deleting with wrong owner
        with self.assertRaises(PermissionDeniedError):
            OsintDataAccessLayer().delete_entity(created.id, owner="another_user", roles=[UserRole.ADMIN])

        # Clean up
        OsintDataAccessLayer().delete_entity(created.id, owner="owner_user", roles=[UserRole.ADMIN])


if __name__ == "__main__":
    unittest.main()
